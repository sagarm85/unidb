...
| 60 | `60_event_queue_serde_json.md` | Performance | 🔄 IN PROGRESS — replace `serde_json::json!` + `row_to_json` (Value AST heap allocation) in `send_event_capture` with manual string builder; VECTOR(128) columns no longer boxed as 128 `JsonValue::Number`s. Target: W4/W0 at 100k ≤ 1.50×. |
