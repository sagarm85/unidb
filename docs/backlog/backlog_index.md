...
| 115 | `115_oneshot_filtered_select_fixed_cost.md` | Performance | 🔄 IN PROGRESS — target one-shot filtered SELECT ≥0.75× (was 0.58×). Step-0 decomposed the 852 µs one-shot premium (global first-use ~590 / per-table ~180 / per-page ~90; plan-cache-miss premium only ~22 µs). Unit 1 shipped 2026-07-24: `Engine::warm_query_path()` open-time warmup (no txn, no WAL) → native one-shot 1,089→744 µs. Remaining: per-table resolve first-use, then per-page madvise if needed. |
| 116 | `116_insert_per_row_commit_path.md` | Performance | 🔄 IN PROGRESS — target per-row INSERT ≥0.75× (was 0.50×). Step-0 2026-07-24: default mode confirmed 1 fsync/commit; software 117 µs/row split (execute ~70, sync-leader ~24-45) via permanent Q116 timers. Shipped: find_or_alloc_page Vec-waste removal, group_fsync FD cache, **catalog persist explicit sync before catalog_root flip (durability hardening)**. Next unit designed (statement-scoped mini-txn bracket merge, −2 WAL records/row) — not yet implemented. |
