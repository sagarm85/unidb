...
| 19 | `19_sql_surface_gaps.md` | Improvement | 🔄 PARTIAL — G1 (CASE/COALESCE/NULLIF), G2-cast (CAST expressions), G3 (UNION/INTERSECT/EXCEPT + chained set-ops), G4 (ORDER BY non-projected), G5 (RETURNING), G8 (SELECT w/o FROM), G10 (IS NULL) SHIPPED. G2-join (FULL OUTER JOIN)/G6/G7 open. G9/G11 delivered under item 30. |
