...
| 19 | `19_sql_surface_gaps.md` | Improvement | 🔄 PARTIAL — G1 (CASE/COALESCE/NULLIF), G2-cast (CAST), G2-join (FULL OUTER JOIN), G3 (UNION/INTERSECT/EXCEPT), G4 (ORDER BY non-projected), G5 (RETURNING), G6 (derived tables), G7 (window functions: ROW_NUMBER/RANK/DENSE_RANK/LAG/LEAD/SUM/AVG/COUNT/MIN/MAX, whole-partition frame), G8 (SELECT w/o FROM), G10 (IS NULL), G-NATURAL (NATURAL JOIN) SHIPPED. Recursive-CTE/cumulative-window-frame deferred (large; out of §1 scope). G9/G11 delivered under item 30. |
...
| 38 | `38_param_type_coercion.md` | Improvement | ✅ SHIPPED 2026-07-20 — lossless coercion in `compare()`: Text↔Int, Text↔Float, Text↔Bool, Int→Float widening; item-38 arms moved before the Float catch-all so `(Float, Text)` pairs reach the parse path. 18 integration tests in `tests/item38_param_coercion.rs`. INSERT write path stays strict. See PROGRESS.md. |
...
| 106 | `106_vector_pgvector_class_tier.md` | Performance | 🔄 IN PROGRESS — Step-0 complete 2026-07-22 (PR #204): recall-vs-ef curve measured (0.90 first met at ef≈120; 601 µs) AND a new dominant lever found — upper-layer HNSW descent is fully uncached (~290 µs, ef-independent, invisible to old counters). Lever order revised: L0' upper-layer cache → L1 graph quality → L3 re-rank pushdown; SQ8 demoted to reserve. Tunable `ef_search` shipped (`UNIDB_HNSW_EF_SEARCH` + `set_ef_search()`). Target ≤400 µs at 10k, recall ≥0.90. |
...
| 121 | `121_auth_core.md` | Milestone | 🔄 IN PROGRESS — Workstream A (P0). **A1+A2 SHIPPED** on branch `claude/permissions-security-supabase-comparison-ixho2w` (`fcd320a`): argon2id credential store + `CREATE USER … PASSWORD` + real password login with the enumeration/timing oracle closed (dummy-hash on miss) and no credential leakage. **A3+A4 SHIPPED** same branch (`77fab4a`): `POST /auth/signup` (gated `UNIDB_ALLOW_SIGNUP`, 404 when off), `POST /auth/refresh` (verify+rotate opaque 256-bit refresh tokens, hash-only session store in `roles.json`), `POST /auth/logout` (idempotent revoke); login/signup return `{token, access_token, refresh_token, expires_in}`. item121 16/16, crash 54/54. Remaining: A5 prod issuer, A6 RS256/JWKS. |
| 122 | `122_rls_token_binding.md` | Milestone | 🔄 IN PROGRESS — Workstream B (P0). **B1+B2 SHIPPED** on branch (`3fb0e04` + QExpr-path fix `2a5fe85`): `auth.uid()` + real `auth.jwt() ->> 'claim'`, substituted at policy-injection time, **fail-closed** (Null, never Bool(true) — item-110); item122 7/7 incl. two-tenant isolation + LIMIT/QExpr path; crash 54/54. **B3+B4 SHIPPED** (`838d91d`): built-in `anon`/`authenticated`/`service_role` roles (service_role RLS bypass audited, item-103), role-scoped policies (`… TO <role>`) with no-`TO` back-compat + fail-closed deny; item122_b3_b4 6/6, crash 54/54. Remaining: B5 column grants (item 112). |
