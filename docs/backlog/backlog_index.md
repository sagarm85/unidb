...
| 118 | _(reserved — file pending)_ | Improvement | 🔄 IN PROGRESS on branch `feat/hnsw-crash-reconcile` — async-HNSW crash-durability hole: a committed vector row whose HNSW insert is still in the in-memory worker channel at crash time is lost from the index (WAL redo restores only durably-applied inserts; no reconciliation). Fix: `hnsw.dirty` clean-shutdown marker + background reconcile-on-open (diff `node_index` vs committed heap, re-enqueue the missing tail) + `DiskHnswIndex::contains` idempotency guard + clean-shutdown drain. Backlog file lands with the branch. |
