...
| 117 | `117_hot_update_unique_key_gate.md` | Performance | 🔄 IMPLEMENTED 2026-07-30 (native + crash green; Docker cert pending) — from the 2026-07-30 architecture review: HOT UPDATE was disabled by the mere *existence* of a PK/UNIQUE index (`!has_unique`), so the common "update a non-indexed column on a PK'd table" case (Table 5 = **0.19×**, worst CRUD row) was forced onto the slow per-row loop with a redundant per-row PK re-check. Fix mirrors item-53's FK gate: `has_unique_in_set` — HOT/enforce_unique only when a unique/PK column is actually in `SET`. Safety verified: unchanged unique index resolves via the same `get_visible` HOT-chain walk as secondary indexes. |
