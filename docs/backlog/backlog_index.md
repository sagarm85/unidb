...
| 19 | `19_sql_surface_gaps.md` | Improvement | 🔄 PARTIAL — G1 (CASE/COALESCE/NULLIF), G2-cast (CAST), G3 (UNION/INTERSECT/EXCEPT), G4 (ORDER BY non-projected), G5 (RETURNING), G6 (derived tables), G7 (window functions: ROW_NUMBER/RANK/DENSE_RANK/LAG/LEAD/SUM/AVG/COUNT/MIN/MAX, whole-partition frame), G8 (SELECT w/o FROM), G10 (IS NULL) SHIPPED. G2-join (FULL OUTER JOIN)/G7-recursive-CTE open. G9/G11 delivered under item 30. |
