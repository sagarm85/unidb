...
12. [ERP app walkthrough — concrete payloads (item 30)](#12-erp-app-walkthrough--concrete-payloads-item-30)
...
## 12. ERP app walkthrough — concrete payloads (item 30)

> **What this section is.** The previous sections define the surface. This section
> shows you how to *use* it end-to-end, walking an ERP schema (customers, products,
> sales orders, invoices, payments, PK/FK-linked with a `VECTOR` search column)
> from schema creation through realtime events and record-browser search. Every
> payload below is real — you can paste it into curl or a JavaScript `fetch` call
> against a running `unidb-server`.
>
> All requests use `POST /sql` (the Milestone-18 boundary: no app-shaped REST
> endpoints). The engine does not know it is an ERP; the caller does.

### 12.1 Auth

Every HTTP request carries a JWT in the `Authorization` header. The server is
**verify-only**: it checks the token's signature and expiry, but does not issue
tokens. Generate them with `scripts/gen_jwt.sh` (in development) or your own
auth service (in production).

```bash
export UNIDB_TOKEN="<your-hs256-jwt>"
export UNIDB_BASE="https://db.example.com:8080"

# Health-check (no body needed — the /sql route is used for all queries below)
curl -sf "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT 1"}'
# → {"results":[{"type":"rows","columns":["1"],"rows":[[1]]}]}
```

See [§1](#1-connect-access--auth) for embed and attach variants.

### 12.2 Schema and foreign keys

One `POST /sql` body can be `;`-separated and is atomic as a whole. Create the
full ERP schema in a single request:

```bash
curl -s "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{
    "sql": "
      ...
      CREATE INDEX idx_customer_embedding ON customers USING HNSW (embedding);
      CREATE INDEX idx_invoice_body ON invoices USING FULLTEXT (body)
    "
  }'
# → {"results":[{"type":"affected","count":0}, ...]} (one entry per DDL statement)
```

> **Note.** `SERIAL` is an identity shorthand that increments automatically; the
> column type is `bigint` in `information_schema.columns`. `REFERENCES` is parsed,
> persisted, and introspectable; referenced-*row* enforcement is a filed follow-up
> (see [§9](#9-honest-limitations)).

### 12.3 ERD introspection — FK edges and index badges

The studio's ERD draws edges from real FK metadata, not from column-name
heuristics. The same query works from a browser, a BFF, or `curl`:
...
**Response** (abbreviated — six FK edges total):
...
```json
{
  "results": [{
    ...
    "rows": [
      ...
      ["invoices",     "order_id",     "sales_orders", "id"],
...
Pair with `unidb_catalog.indexes` to show HNSW / FULLTEXT index badges:

```bash
curl -s "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT table_name, column_name, index_type FROM unidb_catalog.indexes"}'
# → rows: [["customers","embedding","hnsw"], ["invoices","body","fulltext"], ...]
```

### 12.4 Atomic multi-model transaction

This is unidb's differentiating capability: a single row write, its embedding,
and its change event are **one WAL append and one commit** — no dual-write, no
split-brain, no compensating transactions.

Open a session, run several statements in one transaction, commit:

```bash
# Step 1: open a session
TXN=$(curl -sf -X POST "$UNIDB_BASE/txn/begin" \
       -H "Authorization: Bearer $UNIDB_TOKEN" | jq -r '.txn_id')

# Step 2: insert a customer with an embedding vector (one /sql call per statement,
#         each carries X-Txn-Id so they share the same open transaction)
curl -sf "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H "X-Txn-Id: $TXN" \
  -H 'Content-Type: application/json' \
  -d '{
    "sql":    "INSERT INTO customers (name, email, embedding) VALUES ($1, $2, $3)",
    "params": ["Acme Corp", "acme@example.com",
               [0.12, 0.34, 0.56, 0.11, 0.22, 0.33, 0.44, 0.55,
                0.66, 0.77, 0.88, 0.99, 0.10, 0.21, 0.32, 0.43,
                0.54, 0.65, 0.76, 0.87, 0.98, 0.09, 0.18, 0.27,
                0.36, 0.45, 0.54, 0.63, 0.72, 0.81, 0.90, 0.01,
                0.12, 0.23, 0.34, 0.45, 0.56, 0.67, 0.78, 0.89,
                0.90, 0.01, 0.12, 0.23, 0.34, 0.45, 0.56, 0.67,
                0.78, 0.89, 0.90, 0.01, 0.12, 0.23, 0.34, 0.45,
                0.56, 0.67, 0.78, 0.89, 0.90, 0.01, 0.12, 0.23,
                0.34, 0.45, 0.56, 0.67, 0.78, 0.89, 0.90, 0.01,
                0.12, 0.23, 0.34, 0.45, 0.56, 0.67, 0.78, 0.89,
                0.90, 0.01, 0.12, 0.23, 0.34, 0.45, 0.56, 0.67,
                0.78, 0.89, 0.90, 0.01, 0.12, 0.23, 0.34, 0.45,
                0.56, 0.67, 0.78, 0.89, 0.90, 0.01, 0.12, 0.23,
...
curl -sf "$UNIDB_BASE/sql" \
  ...
  -H "X-Txn-Id: $TXN" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"INSERT INTO sales_orders (customer_id, status) VALUES (1, '\''confirmed'\'')"}'
...
curl -sf "$UNIDB_BASE/sql" \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "sql":    "INSERT INTO invoices (order_id, amount, body) VALUES ($1, $2, $3)",
    "params": [1, "1500.00", "Invoice for order 1: consulting services, overdue"]
  }'

# Step 5: commit — the customer row, HNSW vector, order, invoice, and their
#          change events all commit atomically in ONE WAL entry.
curl -sf -X POST "$UNIDB_BASE/txn/$TXN/commit" \
  -H "Authorization: Bearer $UNIDB_TOKEN"
# → {"status":"committed"}
```

**Why this matters vs. Postgres + pgvector + Debezium:**

| | unidb | PG + pgvector + Debezium |
|---|---|---|
| Customer row + embedding | one INSERT | two INSERTs (PG row + pgvector row) |
| Change event capture | same WAL commit | async CDC log-tail (separate process) |
| Atomicity guarantee | yes — one txn, one WAL entry | no — Debezium reads after commit; row visible before event |
| Failure surface | one system | three systems, three failure domains |

### 12.5 Realtime events

Enable event capture for invoices (once per table; idempotent):

```bash
curl -sf -X POST "$UNIDB_BASE/tables/invoices/events" \
  -H "Authorization: Bearer $UNIDB_TOKEN"
# → {"status":"ok"}
```

Subscribe (SSE, Supabase-compatible wire format):

```bash
curl -N "$UNIDB_BASE/events/subscribe?consumer=billing&format=supabase" \
     -H "Authorization: Bearer $UNIDB_TOKEN"
```

**Stream (one frame per committed change):**

```
id: 1
data: {"eventType":"INSERT","new":{"id":1,"order_id":1,"amount":"1500.00","body":"Invoice for order 1...","status":"unpaid","issued_at":"2026-07-14T..."},"old":{},"schema":"public","table":"invoices","commit_timestamp":"2026-07-14T10:00:00.000Z"}

id: 2
data: {"eventType":"UPDATE","new":{"id":1,"status":"paid",...},"old":{"id":1,"status":"unpaid",...},...}
```

Each frame's `id:` is the monotonic `seq` — pass it as `Last-Event-ID` on
reconnect to resume exactly where you left off. Ack once the batch is processed:

```bash
curl -sf -X POST "$UNIDB_BASE/events/ack" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"consumer":"billing","seq":1}'
```

Check lag (survives reconnects; the engine tracks it durably):

```bash
curl -sf "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT consumer, lag_events, lag_seconds FROM unidb_catalog.subscription_lag"}'
# → {"results":[{"type":"rows","columns":["consumer","lag_events","lag_seconds"],
#                "rows":[["billing", 0, 0.0]]}]}
```

See [§8](#8-consume-the-event-stream-change-events) for the full durable-consumer
vs. live-tail model and the vacuum-horizon contract.

### 12.6 Search — vector similarity and full-text

Both searches run over `/sql` — no dedicated search endpoint. The same `xid`
transaction can mix row inserts, vector search, and text search atomically.

**Vector similarity (`NEAR`) — find customers similar to a query vector:**
...
curl -sf "$UNIDB_BASE/sql" \
  ...
  -H 'Content-Type: application/json' \
  -d '{
    "sql":    "SELECT id, name FROM customers WHERE NEAR(embedding, $1, 5)",
    "params": [[0.10, 0.30, 0.50, 0.15, 0.25, 0.35, 0.45, 0.55,
                0.65, 0.75, 0.85, 0.95, 0.10, 0.20, 0.30, 0.40,
                0.50, 0.60, 0.70, 0.80, 0.90, 0.05, 0.15, 0.25,
                0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95, 0.05,
                0.15, 0.25, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85,
                0.95, 0.05, 0.15, 0.25, 0.35, 0.45, 0.55, 0.65,
                0.75, 0.85, 0.95, 0.05, 0.15, 0.25, 0.35, 0.45,
                0.55, 0.65, 0.75, 0.85, 0.95, 0.05, 0.15, 0.25,
                0.35, 0.45, 0.55, 0.65, 0.75, 0.85, 0.95, 0.05,
                0.15, 0.25, 0.35, 0.45, 0.55, 0.65, 0.75, 0.85,
                0.95, 0.05, 0.15, 0.25, 0.35, 0.45, 0.55, 0.65,
                0.75, 0.85, 0.95, 0.05, 0.15, 0.25, 0.35, 0.45,
...
# → {"results":[{"type":"rows","columns":["id","name"],"rows":[[1,"Acme Corp"]]}]}
```

`NEAR(column, vector, k)` returns the k nearest neighbours by L2 distance, using
the HNSW index built at schema-creation time.

**Full-text search (`MATCH`) — find invoices containing specific words (item 30, G11):**

```bash
curl -sf "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT id, body FROM invoices WHERE MATCH(body, '\''invoice overdue'\'')"}'
```
...
```json
{
  "results": [{
    "type": "rows",
    "columns": ["id", "body"],
    "rows": [[1, "Invoice for order 1: consulting services, overdue"]]
  }]
}
```

`MATCH(column, 'query text')` is a boolean `WHERE` predicate. Semantics:
`'invoice overdue'` means AND — rows containing **both** "invoice" and "overdue"
are returned; a row containing only one is not. The FULLTEXT index (built by
`CREATE INDEX … USING FULLTEXT`) is used automatically; if no index exists, the
engine returns `SQL_UNSUPPORTED`. No `tsquery` syntax or configuration is
required — the tokenizer handles whitespace-separated terms.

Both `NEAR` and `MATCH` compose with ordinary SQL predicates in the same `WHERE`
clause:

```sql
-- Overdue invoices with a similar vendor profile
SELECT c.name, i.amount
FROM   invoices i JOIN customers c ON c.id = (
         SELECT customer_id FROM sales_orders WHERE id = i.order_id
       )
WHERE  MATCH(i.body, 'overdue') AND i.status = 'unpaid'
```

### 12.7 Record browser — LIKE filters and cursor paging

The studio record browser sends `LIKE` / `ILIKE` filters typed into the column
search boxes directly as SQL predicates. This is the fix shipped in item 30 (G9):
...
curl -sf "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT id, name, email FROM customers WHERE name LIKE $1", "params":["Acme%"]}'

# contains filter (case-insensitive — ILIKE)
curl -sf "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT id, name FROM customers WHERE name ILIKE $1", "params":["%corp%"]}'

# NOT LIKE — exclude a pattern
curl -sf "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT id, name FROM customers WHERE name NOT LIKE $1", "params":["Test%"]}'
```

**Pattern syntax:** `%` matches any run of characters (including empty); `_`
matches exactly one character. `ILIKE` is case-insensitive; `LIKE` is
case-sensitive. NULL LIKE any-pattern → NULL → the row is excluded (same as
Postgres). The `$n` form is injection-safe — the pattern is data, never SQL.

**Cursor paging** for large tables (avoid deep `OFFSET`):

```bash
# Open a cursor over a large invoice export
CURSOR=$(curl -sf "$UNIDB_BASE/sql" \
  -H "Authorization: Bearer $UNIDB_TOKEN" \
  -H 'Content-Type: application/json' \
  -d '{"sql":"SELECT id, amount, status FROM invoices ORDER BY id", "cursor":true}' \
  | jq -r '.cursor_id')

# Page 1
curl -sf "$UNIDB_BASE/sql/cursor/$CURSOR?limit=50" \
  -H "Authorization: Bearer $UNIDB_TOKEN"
# → {"type":"rows","columns":[...],"rows":[...],"has_more":true}
...
`"cursor":true` materialises nothing server-side — the cursor is a server-side
iterator that pages from the heap. Combine with `WHERE name ILIKE $1` for
filtered paging. See [§5](#5-results-types--paging) for the full paging model.

---

*Item 30 (Studio API readiness): E1 (G9 LIKE/ILIKE) and E2 (G11 MATCH) added
the two predicates that make this walkthrough possible. Item 19 tracks the
remaining SQL surface gaps.*
