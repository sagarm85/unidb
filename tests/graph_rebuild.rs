...
//
// Two CSR-specific analogues of the tests below briefly lived here during
// M7 and were removed during M8 merge verification, alongside the actual
// bug fix in `src/graph/index.rs` (see that file's module comment):
// `edges_from`/Cypher no longer ever consult the CSR graph index, so tests
// asserting they exercise "the CSR-preferring path" would be testing
// `EdgeIndex` again under a misleading name.
